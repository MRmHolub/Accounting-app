...
dfitems = DataFrame(list(BuyItem.objects.all().values()))
    ...
    dbc.Row(
        dbc.Col([
            ...
            dcc.Checklist(id='my-checklist', value=['food', 'school','flat','others'],
                         ...
                         labelClassName="mr-4"),
           dcc.Graph(id='my-pie')], width={'size':5, 'offset':1}))],
...
def display_value(year,month,*args,**kwargs):
    ...
    dff = df[df["customer_id"] == int(person.id)]
    dff = dff[dff["compare_date"].str.contains(date)]
    
    cart_ids=dff["id"].unique()
    kwargs["django_plotly_dash"]=cart_ids

    fig = px.bar(dff, x="date", y="prize", color="item_amount",
    ...
    @app.callback(Output('my-pie', 'figure'),Input('my-checklist', 'value'))
    def display_valus(clients,*args,**kwargs):

        cart_ids=kwargs["django_plotly_dash"]
        dff=dfitems["cart_id"].isin(cart_ids)
